...
from train.training_loop_cosmos import TrainLoop
...
def main():
    ...
    model, diffusion = create_model_and_diffusion(args, motion_data, motion.shape[1])
    model.to(dist_util.dev())
    diffusion.to(dist_util.dev())

    print('Total params: %.2fM' % (sum(p.numel() for p in model.parameters()) / 1000000.0))
    print("Training...")
    TrainLoop(args, train_platform, model, diffusion, data=None).run_loop(motion, labels)
